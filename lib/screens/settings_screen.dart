...
// Update global notifiers — instant app-wide effect
appLanguageNotifier.value   = _lang;
appDateFormatNotifier.value = _date;
appTimeFormatNotifier.value = _time;
...
return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
// Header
_Header('System Preferences',
'Customize the interface and localization settings.'),
const SizedBox(height: 24),
...
'Light Mode', Icons.wb_sunny_outlined,
...
Row(children: [
_LangChip('English', _lang == 'English',
() => setState(() => _lang = 'English')),
const SizedBox(width: 10),
_LangChip('हिन्दी (Hindi)', _lang == 'Hindi',
() => setState(() => _lang = 'Hindi')),
]),
const SizedBox(height: 6),
Text(
_lang == 'Hindi'
? 'ऐप की भाषा हिन्दी में बदल जाएगी'
    : 'App interface will change to English',
style: TextStyle(fontSize: 12, color: c.textMuted,
...
'MM/DD/YYYY',
'YYYY-MM-DD',
].map((fmt) => _FormatOption(
label:    fmt,
example:  _dateExample(fmt),
selected: _date == fmt,
onTap:    () => setState(() => _date = fmt),
)).toList()),
const SizedBox(height: 20),
...
label:    fmt,
example:  _timeExample(fmt),
selected: _time == fmt,
onTap:    () => setState(() => _time = fmt),
)).toList()),
const SizedBox(height: 32),
...
switch (fmt) {
case 'MM/DD/YYYY': return 'e.g. $m/$d/$y';
case 'YYYY-MM-DD': return 'e.g. $y-$m-$d';
default:           return 'e.g. $d/$m/$y';
}
}

String _timeExample(String fmt) {
final now = DateTime.now();
if (fmt == '24-hour') {
return 'e.g. ${now.hour.toString().padLeft(2,'0')}:'
'${now.minute.toString().padLeft(2,'0')}';
}
final h    = now.hour > 12 ? now.hour - 12 : (now.hour == 0 ? 12 : now.hour);
final amPm = now.hour >= 12 ? 'PM' : 'AM';
return 'e.g. $h:${now.minute.toString().padLeft(2,'0')} $amPm';
}
}
